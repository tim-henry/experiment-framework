...
def run(train_loader_fn, test_loader_fn, model_fn, args):
    ...
    for keep_pct in keep_pcts:
        ...
        # if args['use_cuda'] and model.name == "early_branching":
        #     print("MODEL IS BRANCHING")
        #     model.cuda()
        #     model.layer3s = [i.cuda() for i in model.layer3s]
        #     model.layer4s = [i.cuda() for i in model.layer4s]
        #     model.avgpools = [i.cuda() for i in model.avgpools]
